...
function validate( opts, options ) {
	...
	if ( options.hasOwnProperty( 'gamma' ) ) {
		...
		if ( !isPositive( opts.gamma ) ) {
			return new TypeError( 'cdf()::invalid option. `gamma` parameter must be a positive number. Option: `' + opts.gamma + '`.' );
		}
	}
	if ( options.hasOwnProperty( 'x0' ) ) {
		opts.x0 = options.x0;
		if ( !isNumber( opts.x0 ) ) {
			return new TypeError( 'cdf()::invalid option. `x0` parameter must be a number primitive. Option: `' + opts.x0 + '`.' );
		}
	}
